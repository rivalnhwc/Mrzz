...
//    boolean showingRefreshView =false, readyToRefresh =false, Refreshing =false;
...
//        headerView = (LinearLayout) LayoutInflater.from(context).inflate(R.layout.lv_header, null);
...
////        pullToRefreshView.setVisibility(GONE);
////        addHeaderView(pullToRefreshView);
    ...
    private OnTopShowingStateChangeListener topShowingStateChangeListener;
    ...
    public void setOnTopShowingStateChangeListener(OnTopShowingStateChangeListener topShowingStateChangeListener)
    {
        this.topShowingStateChangeListener = topShowingStateChangeListener;
    }

    public interface OnTopShowingStateChangeListener
    {
...
//        });
    ...
    protected void layoutChildren() {
        ...
        try {
            ...
            if (dataChanged) {
                ...
                Log.d(TAG,"fillActiveViews     1111111111111111111111");
            ...
            switch (mLayoutMode) {
                ...
                case LAYOUT_SYNC: {
                    Log.i(TAG,"layoutchildren   mSyncPosition   "+mSyncPosition);
                ...
                default: {
                    if (childCount == 0) {
                        ...
                        Log.i(TAG,"layoutchildren   mFirstPosition   "+mFirstPosition);
            ...
            Log.d(TAG,"mDataChanged = false   1111111111111111");
            mDataChanged = false;
            mNeedSync = false;
            mLayoutMode = LAYOUT_NORMAL;
            invokeOnItemScrollListener();
    ...
    public void resetToTop() {
        ...
        stopFlingRunnable();
        isOnTop=true;
        if(topShowingStateChangeListener!=null)
        topShowingStateChangeListener.OnTopShowingStateChange(isOnTop);
    }
...
//                }
    ...
    private boolean onTouchMove(final MotionEvent event) {
        Log.d("hahahaha", "Y   **************************************"+(int) event.getY());
        if (isRefreshable) {
            ...
            Log.d("hahahaha", "State   **************************************   "+state);
            if (state != REFRESHING && isRecored && state != LOADING) {
                // 保证在设置padding的过程中，当前的位置一直是在head，否则如果当列表超出屏幕的话，当在上推的时候，列表会同时进行滚动
                // 可以松手去刷新了

                if (state == RELEASE_To_REFRESH) {
                    Log.d("hahahaha", "**************************************               "+tempY +"      "+ startY+"             "+headerContentHeight);
                    setSelection(0);
                    // 往上推了，推到了屏幕足够掩盖head的程度，但是还没有推到全部掩盖的地步
                    if (((tempY - startY) / RATIO < headerContentHeight)// 由松开刷新状态转变到下拉刷新状态
                            && (tempY - startY) > 0) {
                        Log.d("hahahaha", "PULL_To_REFRESH   **************************************");
                ...
                if (state == PULL_To_REFRESH) {
                    ...
                    if ((tempY - startY) / RATIO >= headerContentHeight) {// 由done或者下拉刷新状态转变到松开刷新
                        Log.d("hahaha", "state=release to refresh");
        ...
        switch (mTouchMode) {
            ...
            case TOUCH_MODE_DONE_WAITING:
                ...
                Log.d("bbbbb","TOUCH_MODE_DONE_WAITING     y   "+y);
                break;
            case TOUCH_MODE_SCROLLING:
//            case TOUCH_MODE_OVERSCROLL:
                scrollIfNeeded(y);
                Log.d("hahahaha","TOUCH_MODE_SCROLLING     y   "+y);
                break;
        }
...
//    {
//        resetPullToRefreshViewHeight();
//    }

//    private void resetPullToRefreshViewHeight()
//    {
//        ExtendableListView.LayoutParams Params1=(ExtendableListView.LayoutParams) pullToRefreshView.getLayoutParams();
//        Params1.height = 0;



//        pullToRefreshView.setVisibility(GONE);
//        showingRefreshView =false;
//        readyToRefresh =false;
//        Refreshing =false;
//    }

//    private boolean changePullToRefreshViewHeight(int y)
//    {
//        ExtendableListView.LayoutParams Params1=(ExtendableListView.LayoutParams) pullToRefreshView.getLayoutParams();
//        Params1.height += y;
//        if(Params1.height<200) {
//            pullToRefreshView.setVisibility(VISIBLE);
//            pullToRefreshView.setLayoutParams(Params1);
//            readyToRefresh =false;
//            return true;
//        }else if(Params1.height<=0)
//        {
//            pullToRefreshView.setVisibility(GONE);
//            readyToRefresh =false;
//            return false;
    ...
    private boolean onTouchCancel(final MotionEvent event) {
        ...
        if (isRefreshable) {
            if (state != REFRESHING && state != LOADING) {
                ...
                if (state == RELEASE_To_REFRESH) {
                    Log.d("hahahaha","Ontouchcnacel  &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&   state = REFRESHING");
                    state = REFRESHING;
                    changeHeaderViewByState();
                    onLvRefresh();
                }
    ...
    private boolean onTouchUp(final MotionEvent event) {
        if (isRefreshable) {
            if (state != REFRESHING && state != LOADING) {
                ...
                if (state == RELEASE_To_REFRESH) {
                    Log.d("hahahaha","Ontouchupp&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&   state = REFRESHING");
                    state = REFRESHING;
                    changeHeaderViewByState();
                    onLvRefresh();
                }
...
//        }
    ...
    private boolean onTouchUpScrolling(final MotionEvent event) {
        Log.d("hahahaha","onTouchUpScrolling");
        if (isRefreshable) {
            if (state != REFRESHING && state != LOADING) {
                if (state == PULL_To_REFRESH) {
                    state = DONE;
...
//                    if(!showingRefreshView||Refreshing)
...
//                    else atEdge=true;
                ...
                if (motionView != null) {
                    if (atEdge) {
//                        showingRefreshView =changePullToRefreshViewHeight(incrementalDeltaY);
//                            Log.d(TAG,"start fresh");
    ...
    private boolean moveTheChildren(int deltaY, int incrementalDeltaY) {
        ...
        if(state == DONE||state== REFRESHING)
        isRefreshable = false;
        if (DBG) Log.d(TAG, "moveTheChildren can      ScrollDown " + cannotScrollDown);

        Log.d("abcdef","need to load more      itemcount  "+  mItemCount+"       mFirstPosition   "+mFirstPosition);

        if(mItemCount-mFirstPosition<15)
        {
            Log.d("abcdef","need to load more      isLoadingMore  "+  isLoadingMore);
            //need to load more
            if(!isLoadingMore)
            {
                //还没加载过
        ...
        Log.d(TAG, "incrementalDeltaY                " + incrementalDeltaY);
        ...
        if (isDown) {
            ...
            rememberSyncState();
            Log.d(TAG,"mfirstposition   "+mFirstPosition);
        }
    ...
    private View fillDown(int pos, int nextTop) {
        ...
        while ((nextTop < end+end || hasSpaceDown()) && pos < mItemCount) {
            // TODO : add selection support
            if (DBG) Log.d(TAG, "fillDown - pos:" + pos + " nextTop:" + nextTop+"      mItemCount   "+mItemCount+"   end  "+end);
    ...
    private View fillUp(int pos, int nextBottom) {
        ...
        while ((nextBottom > end-end || hasSpaceUp()) && pos >= 0) {
            // TODO : add selection support
    ...
    private View fillFromTop(int nextTop) {
        Log.d(TAG,"fillFromTop      nextTop"+nextTop);
    ...
    private View fillSpecific(int position, int top) {
        Log.d(TAG,"fillspecific          ***************************    "+position+"    top    "+top);
...
//        Log.i("wang","makeandaddview");
    ...
    class RecycleBin {
        ...
        View getActiveView(int position) {
            Log.d(TAG,"mFirstActivePosition   "+mFirstActivePosition+"    position   "+position);
    ...
    void rememberSyncState() {
        Log.d(TAG,"rememberSyncState  "+mFirstPosition);
        if (getChildCount() > 0) {
            mNeedSync = true;
